...
- В `1.5.2-working` clean-install path второй нейросети закрыт bundled runtime: `build.sh` по умолчанию кладет `llama-completion` в `Contents/Resources/bin`, transitive `.dylib` dependencies в `Contents/Resources/lib`, переписывает install names на `@rpath`, подписывает nested Mach-O и проверяет artifact через `scripts/check_bundled_llama_runtime.sh`. Homebrew `llama.cpp` теперь только developer fallback, не пользовательское требование.
