...
| Text improvement ordered-list boundary | если source явно содержит `первое/второе/третье`, Qwen не может создать дополнительный пункт `4.` без маркера `четвёртое`; результат fallback-ится к preformatted Whisper-тексту | Verified by `scripts/test_text_improvement_runner.sh` and regression from `20260509-221100-F5FA772C` |
