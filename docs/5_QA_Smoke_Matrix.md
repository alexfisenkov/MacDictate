...
| Text improvement bundled runtime artifact | собранный `.app` содержит arm64 `llama-completion`, все transitive `.dylib`, nested signatures; `otool` не содержит `/opt/homebrew` / `/usr/local` ссылок; runtime запускается без `dyld` error | Verified by `scripts/check_bundled_llama_runtime.sh`, `./build.sh`, and `scripts/check_distribution_signing.sh build/MacDictate.app` |
| Text improvement runtime missing | app показывает diagnostic `нужен компонент runtime` для второй нейросети, базовая диктовка остается доступной | Compile-level only |
...
| Text improvement fail-closed validator | короткие ответы-галлюцинации, appended answer after source, missing critical numeric/version/URL/email tokens, unexpected list formatting и truncated stdout fallback-ятся к preformatted Whisper-тексту; debug trace пишет fallback reason | Verified by `scripts/test_text_improvement_runner.sh` |
| Text improvement ordered-list boundary retry | если source явно содержит `первое/второе/третье`, а Qwen создает дополнительный пункт `4.` без маркера `четвёртое`, runner делает один controlled retry; валидный retry вставляется, повторно невалидный результат fallback-ится к preformatted Whisper-тексту | Verified by `scripts/test_text_improvement_runner.sh` and regression from `20260509-221100-F5FA772C` |
