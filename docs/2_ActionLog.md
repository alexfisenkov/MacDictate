...
- По debug-сессии `20260509-221100-F5FA772C` найден пограничный list-boundary case: исходник явно содержал `первое/второе/третье`, но Qwen создал дополнительный пункт `4.` из следующего абзаца без маркера `четвёртое`. Добавлен `extra_ordered_list_item` validator: если output numbered-list содержит больший индекс, чем source/preformatted source или явные ordered cues, результат fallback-ится к preformatted source. Одновременно добавлены нормализации `чата GPT -> ChatGPT` и `Cloud Code -> Claude Code`.
