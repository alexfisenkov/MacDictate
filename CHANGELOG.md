...
## [Unreleased] 1.5.2 Working Line

### Notes
- Рабочая линия открыта после публикации `v1.5.1`.
...
- Post-processing второй нейросети дополнительно вырезает служебные преамбулы вроде `Вот исправленный и отформатированный текст:` и standalone Markdown-разделители `---`, если Qwen добавляет их в результат.
- `Syntax AI` / `SyntaxAI` / `Синтакс AI` / `синтакс ай` нормализуются в каноническое `Syntx AI`.
- Ordered-list formatter теперь считает `и второе` / `и третье` частью речевого маркера списка, чтобы не оставлять хвост `И.` в предыдущем пункте.
...
- Technical commentary guard расширен на ответы Qwen вроде `Ваш запрос можно переписать следующим образом` и `Текст сохранен...`: такие служебные преамбулы/футеры больше не попадают во вставку.
- Если Qwen отвечает на продиктованный вопрос как чат-ассистент (`Конечно, я могу помочь...`, `Пожалуйста, предоставьте...`), MacDictate теперь fallback-ится к самому продиктованному вопросу; prompt дополнительно запрещает отвечать на вопросы вместо их коррекции.
...
- Ordered-list validator больше не разрешает Qwen создавать дополнительные пункты списка без исходного речевого маркера (`четвёртое` и т.п.); сначала запускается один controlled retry с причиной ошибки, и только повторно невалидный результат fallback-ится к preformatted Whisper-тексту. Добавлены нормализации `чата GPT -> ChatGPT` и `Cloud Code -> Claude Code`.
