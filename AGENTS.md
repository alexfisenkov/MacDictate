...
- В `1.5.1-working` release-signing контур переведен на Developer ID: `build.sh` поддерживает `MACDICTATE_SIGN_IDENTITY`, hardened runtime, timestamp, DMG signing и `MACDICTATE_NOTARY_PROFILE` / `MACDICTATE_NOTARIZE=true`; runbook — `docs/9_Distribution_Signing_Runbook.md`. Первый notarized/stapled DMG подтвержден с `Developer ID Application: Aleksander Fisenkov (5BABN9U6WS)` и notary profile `macdictate-notary`.
